<template>
  ...
  >
    <form method="get" class="quick-search-form">
      <div class="input-group spinner-input spinner-sm spinner-brand spinner-right" :class="{ spinner: loading }">
        <div class="input-group-prepend">
          <span class="input-group-text">
            <i class="flaticon2-search-1"></i>
          </span>
        </div>
        <input @keyup="onSearch" type="text" class="form-control quick-search-input" placeholder="Search..." />
        <div class="input-group-append">
          <span class="input-group-text">
            <i class="quick-search-close" @click="reset" :style="{ display: hasResult() ? 'flex' : '' }"></i>
          </span>
        </div>
      </div>
    </form>
...
<script>
import KTSearchResult from "@/core/layout/extras/dropdown/SearchResult.vue";
